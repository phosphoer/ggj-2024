...
public class BirdPerceptionComponent : MonoBehaviour
{
  ...
  {
    PlayerActorController playerActor= PlayerActorController.Instance;

    if (playerActor == null)
      return;

    Transform playerTransform = playerActor.AIVisibilityTarget != null ? playerActor.AIVisibilityTarget : playerActor.transform;
    Vector3 playerLocation = playerTransform.position;
    float actorDistance = Vector3.Distance(transform.position, playerLocation);

    _canHearPlayer = actorDistance < HearingDistance;
  ...
  {
    if (PlayerActorController.Instance == null)
      return;

    Transform playerTransform = PlayerActorController.Instance.AIVisibilityTarget;
    Vector3 playerLocation = playerTransform.position;
